  ...
  <div>
    ...
    <Teleport to="body">
      <div v-if="isSessionExpired" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 backdrop-blur-md">
        <div class="bg-fantasy-panel border-2 border-epic-red p-8 sketch-panel max-w-sm w-full mx-4 text-center relative overflow-hidden">
          ...
          <p class="text-stone-500 text-sm mb-6 leading-relaxed">您的連線憑證已逾時，<br>請重新登入以繼續您的冒險旅程。</p>
          <button
            @click="login"
            :disabled="!isReady"
            class="w-full py-3 px-6 bg-transparent border-2 border-epic-red text-epic-red hover:bg-epic-red hover:text-white transition-all duration-300 sketch-btn font-serif tracking-wider uppercase disabled:opacity-50 disabled:cursor-not-allowed"
          >
            重新登入
          </button>
        </div>
