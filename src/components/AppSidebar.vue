...
const navClass = (tab, activeText, activeBorder, activeBg) => {
  ...
  return props.currentTab === tab
    ...
    : `${base} text-stone-500 hover:text-stone-900 hover:bg-stone-100`;
};
</script>

<template>
  <aside class="hidden md:flex md:flex-col w-60 shrink-0 bg-fantasy-panel border-r-2 border-stone-500 md:sticky md:top-0 md:h-screen md:overflow-y-auto">

    <!-- Logo -->
    <div class="px-5 py-4 border-b-2 border-stone-400">
      <h1 class="font-serif text-epic-red text-lg tracking-widest">Life Game</h1>
      <p class="text-xs text-stone-400 mt-0.5">上次登入：{{ formatDate(userStats?.Last_Login) }}</p>
    </div>

    <!-- Character -->
    <div class="px-5 py-4 border-b-2 border-stone-400 space-y-3">
      <div class="flex items-center gap-3">
        <div class="w-11 h-11 rounded-full border-2 border-epic-red flex items-center justify-center bg-stone-100 shadow-[0_0_12px_rgba(241,111,79,0.2)] relative shrink-0">
        ...
        <div>
          <div class="text-stone-900 font-serif text-sm">等級 {{ userStats?.Level || 1 }}</div>
          <div class="text-xs text-stone-400">英雄</div>
        </div>
      </div>
      <div>
        ...
        <div class="h-1.5 bg-stone-200 rounded-full overflow-hidden">
          <div class="h-full bg-epic-red transition-all duration-1000 shadow-[0_0_6px_rgba(241,111,79,0.4)]" :style="{ width: expPercentage }"></div>
        </div>
        <div class="text-right text-xs text-stone-400 mt-0.5">總計 {{ userStats?.EXP || 0 }}</div>
      </div>
    </div>

    <!-- Stats -->
    <div class="px-4 py-3 border-b-2 border-stone-400 grid grid-cols-3 gap-2">
